...
class turtlebot():
    ...
    def move2goal(self):
        ...
        self.velocity_publisher.publish(vel_msg)
        if not updatePos :
            self.posex=self.posex+vel_msg.linear.x*cos(self.posetheta)*(rospy.Time.now().to_sec()-timeold);
            self.posey=self.posey+vel_msg.linear.x*sin(self.posetheta)*(rospy.Time.now().to_sec()-timeold);
        else :
            self.posetheta=(self.posetheta+vel_msg.angular.z*(rospy.Time.now().to_sec()-timeold))%(2*pi)"""
