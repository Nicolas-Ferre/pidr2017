...
float getMeterDistanceOfGreyLevel(int grayLevel)
{
        return grayLevel / 255. * DEPTH_CLAMP_VALUE / 1000;
}
...
int main(int argc, char **argv)
{
        ...
        std::vector<float> lineDepth(width, 0);

#ifndef ZED_ONLY
        ros::init(argc, argv, "cam");
        ros::NodeHandle n;
        ros::Publisher pub = n.advertise<std_msgs::Float32MultiArray>("claques", 10);
#endif
...
#endif
        {
                ...
                for (int x = 0; x < width && count > 0; ++x)
                {
                        for (int y = 0; y < height; ++y)
                        {
                                ...
                                {
                                        cv::Vec4b color = colorImage.at<cv::Vec4b>(cv::Point(x, y));
                                        int value = ((int)color[0] + (int)color[1] + (int)color[2]) / 3;
                                        colorImage.at<cv::Vec4b>(cv::Point(x, y)) = cv::Vec4b(value/2, value/2, value/2, 1);
                                }
                        }
                }


                // Calcul de la moyenne de profondeur de la ligne horizontale centrale
                for (int i = 0; i < (int)lineDepth.size(); ++i)
                {
                        lineDepth[i] = lineDepth[i] * lineDepthFrame + getMeterDistanceOfGreyLevel(255 - depthImage.at<uchar>(cv::Point(4 * i, height / 2)));
                        lineDepth[i] /= lineDepthFrame + 1;
                }

                if (++lineDepthFrame == 20)
                {
#ifndef ZED_ONLY
                        // Envoi de la ligne de profondeur
                        std_msgs::Float32MultiArray msg;
                        msg.layout.dim.push_back(std_msgs::MultiArrayDimension());
                        msg.layout.dim[0].size = lineDepth.size();
                        msg.layout.dim[0].stride = 1;
                        msg.layout.dim[0].label = "depth";
                        msg.data.clear();
                        msg.data.insert(msg.data.end(), lineDepth.begin(), lineDepth.end());

                        pub.publish(msg);


                        // Reset
                        lineDepth = std::vector<float>(width, 0);
                        lineDepthFrame = 0;
#endif
                }
                ...
                // Envoi de la profondeur de l'objet le plus proche
                /*if (globalMinDistance < 256)
                {
                        std_msgs::String msg;
                        msg.data = std::to_string(getMeterDistanceOfGreyLevel(globalMinDistance));
